...
export async function init({ THREE: _THREE, scene, camera, renderer, spark }) {
  ...
  function getTransitionModifier(inTransition, fadeIn, t, splatScale, spereRadius, sphereHeight) {
    const contraction = contractionDyno();
    return dyno.dynoBlock({ gsplat: dyno.Gsplat }, { gsplat: dyno.Gsplat }, ({ gsplat }) => {
      gsplat = contraction.apply({ gsplat, inTransition, fadeIn, t, splatScale, spereRadius, sphereHeight }).gsplat;
      return { gsplat };
    });
  }

  async function morphableSplatMesh(assetName, time, fadeInTime, fadeOutTime, period, splatCoverage, spereRadius, sphereHeight) {
    const url = await getAssetFileURL(assetName);
    const splatMesh = new SplatMesh({ url });
    await splatMesh.initialized;
    const splatScale = dyno.div(dyno.mul(splatCoverage, spereRadius), dyno.dynoFloat(splatMesh.packedSplats.numSplats / 1000.0));
    const { inTransition, isFadeIn, normT } = getTransitionState(time, fadeInTime, fadeOutTime, period);
    splatMesh.worldModifier = getTransitionModifier(inTransition, isFadeIn, normT, splatScale, spereRadius, sphereHeight);
  ...
  function update(dt, t) {
    if (!PARAMETERS.pause) {
      ...
      if (PARAMETERS.rotation) meshes.forEach(m => { m.rotation.y += dt * PARAMETERS.speedMultiplier; });
    }
  ...
  function setupGUI(folder) {
    folder.add(PARAMETERS, "spereRadius", 0.1, 8.0, 0.01).onChange(v => spereRadiusDyno.value = v);
    folder.add(PARAMETERS, "sphereHeight", -1.0, 4.0, 0.01).onChange(v => sphereHeightDyno.value = v);
    folder.add(PARAMETERS, "splatCoverage", 0.1, 2.0, 0.01).onChange(v => splatCoverageDyno.value = v);
