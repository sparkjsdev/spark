...
import { dyno, SparkRenderer, SplatMesh, textSplats, SparkControls } from "@sparkjsdev/spark";
...
export async function init({ THREE: _THREE, scene, camera, renderer, spark }) {
    ...
    return dyno.dynoBlock(
      ...
      ({ gsplat }) => {
        const physicsShader = new dyno.Dyno({
          inTypes: { gsplat: dyno.Gsplat, time: "float", dropTime: "float", dropProgress: "float", gravity: "float", bounceDamping: "float", floorLevel: "float", randomFactor: "float", reformSpeed: "float", cycleDuration: "float", friction: "float", shrinkSpeed: "float", explosionStrength: "float", isReforming: "float", reformTime: "float", reformDuration: "float" },
        ...
        gsplat = physicsShader.apply({ gsplat, time: animationTime, dropTime: uDropTime, dropProgress: uDropProgress, gravity: uGravity, bounceDamping: uBounceDamping, floorLevel: uFloorLevel, randomFactor: uRandomFactor, reformSpeed: uReformSpeed, cycleDuration: uCycleDuration, friction: uFriction, shrinkSpeed: uShrinkSpeed, explosionStrength: uExplosionStrength, isReforming: uIsReforming, reformTime: uReformTime, reformDuration: uReformDuration }).gsplat;
        return { gsplat };
      }
    );
  }
    ...
    return dyno.dynoBlock(
      ...
      ({ gsplat }) => {
        const birthShader = new dyno.Dyno({
          inTypes: { gsplat: dyno.Gsplat, time: "float", isBirthing: "float", birthTime: "float", birthDuration: "float" },
          outTypes: { gsplat: dyno.Gsplat },
          globals: () => [dyno.unindent(`
            float hash(vec3 p) { return fract(sin(dot(p, vec3(127.1, 311.7, 74.7))) * 43758.5453); }
          `)],
        ...
        gsplat = birthShader.apply({ gsplat, time: animationTime, isBirthing: uIsBirthing, birthTime: uBirthTime, birthDuration: uBirthDuration }).gsplat;
        return { gsplat };
      }
    );
  }
  ...
  async function loadSplats() {
    ...
    for (const splatName of splatNames) {
      ...
      if (nameKey === 'woobles') mesh.scale.set(1.7, 2.0, 1.7); else mesh.scale.set(1, 1, 1);
  ...
  async function loadTable() {
    ...
    await new Promise((resolve, reject) => {
      loader.load(tableURL, (gltf) => {
        ...
        tableModel.traverse((child) => { if (child.isMesh) { child.castShadow = true; child.receiveShadow = true; } });
  ...
  function switchToSplat(name) {
    Object.values(splatMeshes).forEach(m => { if (m) m.visible = false; });
    if (splatMeshes[name]) { splatMeshes[name].visible = true; currentSplatName = name; }
  }
  ...
  const transitionState = { isTransitioning: false, transitionTime: 0.0, transitionDuration: 3.0 };

  function startExplosion() {
    if (transitionState.isTransitioning) return;
    transitionState.isTransitioning = true;
    transitionState.transitionTime = 0.0;
    Object.entries(splatMeshes).forEach(([name, mesh]) => { if (mesh) mesh.visible = (name === currentSplatName); });
  ...
  function startTransition() {
    ...
    if (splatMeshes[nextSplatName]) {
      Object.entries(splatMeshes).forEach(([name, mesh]) => { if (!mesh) return; if (name !== currentSplatName && name !== nextSplatName) mesh.visible = false; });
  ...
  // Instructional text
  const instructionsText = textSplats({ text: "WASD + mouse to move\nSPACEBAR: Explosion!", font: "Arial", fontSize: 24, color: new THREE.Color(0xFFFFFF), textAlign: "center", lineHeight: 1.3 });
  ...
  function onKeyDown(e) {
    if (e.code === 'Space') { e.preventDefault(); if (transitionState.isTransitioning) completeTransition(); startTransition(); totalTime = 0; }
  }
  ...
  function update(dt, t) {
    ...
    if (transitionParams.autoTransition) {
      ...
      if (!transitionState.isTransitioning && totalTime >= 1.0) { startTransition(); totalTime = 0; }
    }
    if (transitionState.isTransitioning) {
      transitionState.transitionTime += dt;
      uBirthTime.value = transitionState.transitionTime;
      if (transitionState.transitionTime >= transitionState.transitionDuration) completeTransition();
    }
    uDropTime.value += dt;
