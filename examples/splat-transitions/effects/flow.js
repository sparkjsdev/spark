...
export async function init({ THREE: _THREE, scene, camera, renderer, spark }) {
  ...
  const centers = meshes.map(m => {
    ...
    m.packedSplats.forEachSplat((_, c) => { box.expandByPoint(c); });
  ...
  function updateCamera() {
    ...
    const tgt = new THREE.Vector3().lerpVectors(meshes[cur].position, meshes[nxt].position, Math.pow(s, 5));
  ...
  function update(dt, t) {
    if (!PARAMETERS.pause) {
      ...
      if (PARAMETERS.objectRotation) meshes.forEach(m => m.rotation.y += dt * PARAMETERS.speedMultiplier * 2);
    }
    updateCamera();
  }
