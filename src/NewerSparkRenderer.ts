...
export interface NewerSparkRendererOptions {
  /**
...
export class NewerSparkRenderer extends THREE.Mesh {
  renderer: THREE.WebGLRenderer;
  premultipliedAlpha: boolean;
  material: THREE.ShaderMaterial;
  uniforms: ReturnType<typeof NewerSparkRenderer.makeUniforms>;
  ...
  constructor(options: NewerSparkRendererOptions) {
    const uniforms = NewerSparkRenderer.makeUniforms();
  ...
  static makeUniforms() {
    const uniforms = {
      ...
      ordering: { type: "t", value: NewerSparkRenderer.emptyOrdering },
  ...
  onBeforeRender(
    ...
    this.uniforms.ordering.value =
      this.orderingTexture ?? NewerSparkRenderer.emptyOrdering;
  ...
  private updateInternal({
    ...
    //   if (Math.random() < 0.1) {
    //     if (mappingVersion === this.current.mappingVersion) {
    //       console.log(`MATCHES mappingVersion=${mappingVersion}, current.mappingVersion=${this.current.mappingVersion}`);
    //     } else {
    //       console.log(`not mappingVersion=${mappingVersion}, current.mappingVersion=${this.current.mappingVersion}`);
    //     }
    //   }
    // }

    // if (needsUpdate && (mappingVersion !== this.current.mappingVersion) && this.sorting) {
  ...
  private async driveSort() {
    ...
    if (!this.orderingTexture) {
      ...
      // console.log('Updating numLayers', numLayers);
  ...
  private async driveLod({
    ...
    this.lodWorker.tryExclusive(async (worker) => {
      ...
      if (this.lodDirty || viewChanged) {
        // console.log(`*** LOD: lodDirty=${this.lodDirty}, viewChanged=${viewChanged}`);
  ...
  private async updateLodInstances(
    ...
    const maxSplats = isAndroid() ? 500000 : isIos() ? 500000 : 1500000;
  ...
  private updateLodIndices(
    ...
    for (const [uuid, countIndices] of Object.entries(keyIndices)) {
      ...
      if (!instance) {
        ...
        //   const props = renderer.properties.get(instance.texture) as { __webglTexture: WebGLTexture };
        //   const glTexture = props.__webglTexture;
        //   if (!glTexture) {
