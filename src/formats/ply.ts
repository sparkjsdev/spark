...
export async function unpackPly<T>(
  ...
  splatEncoder.allocate(numSplats, ply.numShBands);

  ply.parseSplats(splatEncoder);
...
export class PlyReader {
  ...
  async parseHeader() {
    ...
    if (this.elements.vertex) {
      ...
      this.numShBands = getNumSh(this.elements.vertex.properties);
    }
    if (this.elements.sh) {
      this.numShBands = getNumSh(this.elements.sh.properties);
    }
  }
  ...
  parseSplats<T>(splatEncoder: SplatEncoder<T>) {
    ...
    let numSh = 0;
    let shProps: number[] = [];
    let sh: Float32Array | undefined = undefined;

    function prepareSh() {
      // Prepare SH coefficient names and arrays for numSh total SH levels
      const num_f_rest = SH_DEGREE_TO_NUM_COEFF[numSh];
      const sh1Props = new Array(3)
        .fill(null)
        .flatMap((_, k) => [0, 1, 2].map((_, d) => k + (d * num_f_rest) / 3));
      const sh2Props = new Array(5)
        .fill(null)
        .flatMap((_, k) =>
          [0, 1, 2].map((_, d) => 3 + k + (d * num_f_rest) / 3),
        );
      const sh3Props = new Array(7)
        .fill(null)
        .flatMap((_, k) =>
          [0, 1, 2].map((_, d) => 8 + k + (d * num_f_rest) / 3),
        );
      shProps = [...sh1Props, ...sh2Props, ...sh3Props];
      sh = numSh >= 1 ? new Float32Array(num_f_rest) : undefined;
    }
    ...
    function ssShCallback(
      ...
      const fRest = item.f_rest as number[];
      for (let i = 0; i < fRest.length; i++) {
        sh[i] = (fRest[shProps[i]] * 8) / 255 - 4;
      }
      splatEncoder.setSplatSh(index, sh);
    }
...
function getNumSh(properties: Record<string, PlyProperty>) {
  ...
  const numSh = NUM_COEFF_TO_SH_DEGREE[num_f_rest];
  if (numSh == null) {
